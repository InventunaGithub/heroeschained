...
namespace CodeStage.AntiCheat.EditorCode
{
	...
	internal static class SettingsGUI
	{
		...
		private static void DrawIL2CPPSection()
		{
			...
			{
				var fold = GUITools.DrawFoldHeader("IL2CPP is your friend", ACTkEditorPrefsSettings.IL2CPPFoldout);
				if (changed.changed)
				{
		...
		private static void DrawHashSection()
		{
			using (var changed = new EditorGUI.ChangeCheckScope())
			{
				var fold = GUITools.DrawFoldHeader("Code Hash Generator", ACTkEditorPrefsSettings.HashFoldout);
				if (changed.changed)
				{
		...
		private static void DrawSymbol(ref bool field, string symbol, string hint)
		{
			...
			if (EditorGUI.EndChangeCheck())
			{
				if (field)
				{
					SettingsUtils.SwitchSymbol(symbol, true);
				}
				else
				{
					SettingsUtils.SwitchSymbol(symbol, false);
				}
			}
		}
	}
